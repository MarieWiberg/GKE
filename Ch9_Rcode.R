...
plot(0:50, eq.epan.b4$eqYx, type="l", lwd=2.0,ylab=expression(varphi(x)),
     xlab=expression(x),lty=1)
lines(0:50, eq.epan.dkb$eqYx, type="l", lwd=2.0,lty=2)
legend("topleft",lty=c(1,2),c("Beta4","Binomial Kernel"))
...
# Below plot code is not as code in the book
plot(0:50, eq.epan.b4$eqYx, type="l", lty=1,ylab=expression(varphi(x)),
     xlab=expression(x),lwd=2.0)
lines(0:50, eq.adap.b4$eqYx, type="l", lwd=2.0,lty=2)
legend("topleft",lty=c(1,2),c("Epanechnikov","Adaptive"))
...
## 9.7.2 Bootstrap Standard Error of Equating
